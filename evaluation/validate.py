...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate MiniMind TP x CP x PP parallelism"
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help="enable context parallelism; --cp_size alone does not enable it",
    )
...
def expected_dense_tensor(
    ...
    if tp_context.vocab_parallel and name.endswith(
        ("model.embed_tokens.weight", "lm_head.weight")
    ):
        tensor = tensor.chunk(tp_context.world_size, dim=0)[tp_context.rank]
    elif name.endswith(COLUMN_PARALLEL_SUFFIXES):
        tensor = tensor.chunk(tp_context.world_size, dim=0)[tp_context.rank]
...
def get_pipeline_loss(
    ...
    if pp_context.is_last:
        ...
        if cp_context is not None:
            dist.all_reduce(loss_sum, group=cp_context.group)
            dist.all_reduce(num_tokens, group=cp_context.group)
...
def compare_pipeline_logits(
    ...
    if pp_context.is_last:
        ...
        if tp_context.vocab_parallel:
            gathered = [
                torch.empty_like(pipeline_logits)
                for _ in range(tp_context.world_size)
            ]
            dist.all_gather(gathered, pipeline_logits, group=tp_context.group)
            pipeline_logits = torch.cat(gathered, dim=-1)
        if cp_context is not None:
            gathered = [
                torch.empty_like(pipeline_logits)
                for _ in range(cp_context.world_size)
            ]
            dist.all_gather(gathered, pipeline_logits, group=cp_context.group)
            pipeline_logits = torch.cat(gathered, dim=1)
...
def gather_gradient_metrics(
    ...
    payload = (pp_rank, metrics) if cp_rank == 0 and tp_rank == 0 else None
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("The parallel validation requires CUDA")
    ...
    assert args.pp_size >= 1, "pp_size must be positive"
    assert args.tp_size >= 1, "tp_size must be positive"
    assert args.cp_size >= 1, "cp_size must be positive"
    ...
    assert world_size == args.pp_size * effective_cp_size * args.tp_size, (
        "world_size must equal pp_size * effective_cp_size * tp_size"
    )
    ...
    if args.sequence_parallel:
        assert args.seq_len % (effective_cp_size * args.tp_size) == 0
    if args.cp:
        assert args.seq_len % effective_cp_size == 0
    if args.cp and args.cp_comm_type == "a2a":
        assert (config.num_attention_heads // args.tp_size) % effective_cp_size == 0
        assert (config.num_key_value_heads // args.tp_size) % effective_cp_size == 0

    dense_model = MiniMindForCausalLM(config).to(device=device, dtype=dtype)
    stage_model = PipelineStage(config, pp_context, tp_context, cp_context).to(
        device=device,
        dtype=dtype,
    )
