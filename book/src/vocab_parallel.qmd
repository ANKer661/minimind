...
<details>
<summary>Vocab Parallel Cross Entropy 的 forward/backward 关键逻辑</summary>
...
<details>
<summary>Vocab size benchmark 命令</summary>
```bash
...
{
  const rows = [
    {vocab: "6,400", ddp: 4148.79, tp: 2781.92, vp: 2554.47},
    {vocab: "12,800", ddp: 4625.41, tp: 3237.01, vp: 2782.34},
    {vocab: "25,600", ddp: 5577.66, tp: 4147.36, vp: 3237.44},
    {vocab: "51,200", ddp: 7476.97, tp: 5971.36, vp: 4144.78}
  ]
  const vocabDomain = rows.map(d => d.vocab)
  const modeDomain = ["DDP", "TP", "TP + VP"]
  const data = rows.flatMap(d => [
    {vocab: d.vocab, mode: "DDP", value: d.ddp, change: 0},
    {vocab: d.vocab, mode: "TP", value: d.tp, change: 0},
    {vocab: d.vocab, mode: "TP + VP", value: d.vp, change: (1 - d.vp / d.tp) * 100}
  ])
  ...
  const chart = Plot.plot({
    ...
    marks: [
      ...
      Plot.text(data.filter(d => d.mode !== "TP + VP"), {
        fx: "vocab",
        x: "mode",
        y: "value",
        text: d => Math.round(d.value).toLocaleString("en-US"),
        dy: -9,
        fill: d => d.mode === "DDP" ? "#343a40" : "#1971c2",
        fontWeight: 600
      }),
      Plot.text(data.filter(d => d.mode === "TP + VP"), {
        fx: "vocab",
...
{
  const rows = [
    {vocab: "6,400", tp: 129.46, vp: 132.94},
    {vocab: "12,800", tp: 133.59, vp: 136.28},
    {vocab: "25,600", tp: 144.10, vp: 142.03},
    {vocab: "51,200", tp: 165.04, vp: 155.96}
  ]
  const vocabDomain = rows.map(d => d.vocab)
  const modeDomain = ["TP", "TP + VP"]
  const data = rows.flatMap(d => [
    {vocab: d.vocab, mode: "TP", value: d.tp, change: 0},
    {vocab: d.vocab, mode: "TP + VP", value: d.vp, change: (d.vp / d.tp - 1) * 100}
  ])
  ...
  const chart = Plot.plot({
    ...
    y: {label: "Step time (ms)", domain: [0, 185], grid: true},
    color: {
      domain: modeDomain,
      range: ["#1971c2", "#5f3dc4"],
      legend: true
