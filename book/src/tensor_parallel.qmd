...
显存下降的代价是更长的 step time。本实验中 TP 比 DDP 慢约 50%～61%，这部分差距主要来自 Column Parallel 与 Row Parallel 在 forward/backward 中引入的 collective。后面的 @sec-tp-async 会尝试隐藏其中一部分通信；在此之前，[Sequence Parallelism](sequence_parallel.qmd) 先处理另一个更直接的问题：TP 区域之间仍然重复保存的完整 hidden states。
